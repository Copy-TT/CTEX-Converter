import { translations } from "./locales.js?v=0.5";
...
const state = { file: null, image: null, width: 0, height: 0, zoom: 1, language: "en" };
const $ = (id) => document.getElementById(id);
const elements = {
  openPng: $("open-png"), openCtex: $("open-ctex"), clearFile: $("clear-file"), pngInput: $("png-input"), ctexInput: $("ctex-input"), exportPng: $("export-png"), exportCtex: $("export-ctex"), dropZone: $("drop-zone"), previewPanel: $("preview-panel"), canvas: $("preview-canvas"), canvasWrap: $("canvas-wrap"), status: $("status"), fileName: $("file-name"), fileFormat: $("file-format"), fileSize: $("file-size"), resolution: $("resolution"), zoomIn: $("zoom-in"), zoomOut: $("zoom-out"), zoomReset: $("zoom-reset"), languageSelect: $("language-select")
};
...
elements.pngInput.addEventListener("change", async (event) => { const file = event.target.files[0]; event.target.value = ""; await openPng(file); });
...
function setLanguage(language) {
  ...
  for (const [element, key] of [[elements.openPng, "openPng"], [elements.openCtex, "openCtex"], [elements.clearFile, "clearFile"], [elements.exportPng, "exportPng"], [elements.exportCtex, "exportCtex"]]) element.textContent = t(key);
  elements.zoomIn.setAttribute("aria-label", t("zoomIn")); elements.zoomOut.setAttribute("aria-label", t("zoomOut")); elements.languageSelect.value = language;
  if (!state.file) setStatus(t("start"));
}
async function openDroppedFile(file) { if (!file) return; if (file.name.toLowerCase().endsWith(".ctex")) await openCtex(file); else await openPng(file); }
async function openPng(file) { if (!file) return; try { if (file.type && file.type !== "image/png") throw new Error(t("pngOnly")); setDocument(file, await decodeImage(file, t("pngRead")), "PNG"); setStatus(t("opened", { name: file.name })); } catch (error) { setStatus(error.message, true); } }
async function openCtex(file) { if (!file) return; try { const bytes = new Uint8Array(await file.arrayBuffer()); const payload = validateWebpCtex(bytes); const image = await decodeImage(new Blob([payload], { type: "image/webp" }), t("webpRead")); if (image.naturalWidth !== readUint32(bytes, 8) || image.naturalHeight !== readUint32(bytes, 12)) throw new Error(t("mismatchedResolution")); const pixelFormat = readUint32(bytes, 48) === IMAGE_FORMAT_RGBA8 ? "RGBA8" : "RGB8"; setDocument(file, image, `CTEX (GST2 / WebP / ${pixelFormat})`); setStatus(t("opened", { name: file.name })); } catch (error) { setStatus(error.message, true); } }
function validateWebpCtex(bytes) { if (bytes.length < CTEX_WEBP_PAYLOAD_OFFSET) throw new Error(t("fileTooSmall")); if (readAscii(bytes, 0, 4) !== "GST2") throw new Error(t("notGst2")); if (readUint32(bytes, 4) !== 1) throw new Error(t("unsupportedVersion")); if (readUint32(bytes, 36) !== DATA_FORMAT_WEBP) throw new Error(t("unsupportedCtex")); const width = readUint32(bytes, 8); const height = readUint32(bytes, 12); if (!width || !height || readUint16(bytes, 40) !== width || readUint16(bytes, 42) !== height) throw new Error(t("invalidResolution")); const pixelFormat = readUint32(bytes, 48); if (pixelFormat !== IMAGE_FORMAT_RGB8 && pixelFormat !== IMAGE_FORMAT_RGBA8) throw new Error(t("unsupportedPixelFormat")); if (readUint32(bytes, 52) !== bytes.length - CTEX_WEBP_PAYLOAD_OFFSET) throw new Error(t("invalidPayloadLength")); if (readAscii(bytes, 56, 4) !== "RIFF" || readAscii(bytes, 64, 4) !== "WEBP") throw new Error(t("invalidWebp")); return bytes.slice(CTEX_WEBP_PAYLOAD_OFFSET); }
function setDocument(file, image, format) { state.file = file; state.image = image; state.width = image.naturalWidth; state.height = image.naturalHeight; state.zoom = fitZoom(); render(); elements.fileName.textContent = file.name; elements.fileFormat.textContent = format; elements.fileSize.textContent = formatBytes(file.size); elements.resolution.textContent = `${state.width} × ${state.height}`; setEnabled(true); elements.dropZone.hidden = true; }
function clearDocument() { state.file = null; state.image = null; state.width = 0; state.height = 0; state.zoom = 1; elements.canvas.width = 0; elements.canvas.height = 0; elements.canvas.style.width = ""; elements.canvas.style.height = ""; elements.canvasWrap.hidden = true; elements.dropZone.hidden = false; elements.fileName.textContent = "-"; elements.fileFormat.textContent = "-"; elements.fileSize.textContent = "-"; elements.resolution.textContent = "-"; setEnabled(false); setStatus(t("cleared")); }
function render() { elements.canvas.width = state.width; elements.canvas.height = state.height; context.clearRect(0, 0, state.width, state.height); context.drawImage(state.image, 0, 0, state.width, state.height); setZoom(state.zoom); elements.canvasWrap.hidden = false; }
...
function baseName(name) { return name.replace(/\.(png|ctex)$/i, "") || "image"; }
function setEnabled(enabled) { for (const element of [elements.clearFile, elements.exportPng, elements.exportCtex, elements.zoomIn, elements.zoomOut, elements.zoomReset]) element.disabled = !enabled; }
