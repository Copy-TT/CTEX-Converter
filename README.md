...
- **Personal settings** — Switch between Light and Dark themes. The selected theme, interface language, and JPG background are remembered in local browser storage and restored when the page opens.
...
- **개인 설정** — 라이트·다크 테마를 전환할 수 있습니다. 선택한 테마, 인터페이스 언어 및 JPG 배경색은 브라우저 로컬 저장소에 기억되며 페이지를 열 때 복원됩니다.
